...
import { exportTokEntriesCsv } from "../../../lib/csv";
import type { TokEntry } from "../../../lib/types";
...
export default function TokPage() {
    return (
        <Suspense fallback={<div className="p-8">Loading TOK data...</div>}>
            <TokContent />
        </Suspense>
    );
}
...
function TokContent() {
    const { ready, tokEntries, actions } = useDb();
    const searchParams = useSearchParams();
    const [modalOpen, setModalOpen] = useState(false);
    const [editingEntry, setEditingEntry] = useState<TokEntry | null>(null);
    ...
    const handleDelete = async (entryId: string) => {
        if (!confirm("Delete this TOK entry?")) {
            return;
        }
        await actions.deleteTokEntry(entryId);
    };

    const handleExport = () => {
        const csv = exportTokEntriesCsv(tokEntries);
        const blob = new Blob([csv], { type: "text/csv" });
        const url = URL.createObjectURL(blob);
        const link = document.createElement("a");
        link.href = url;
        link.download = "tok-entries.csv";
        link.click();
        URL.revokeObjectURL(url);
    };

    const handleSubmit = async (entry: TokEntry) => {
        if (editingEntry) {
            await actions.updateTokEntry(entry);
        } else {
            await actions.createTokEntry(entry);
        }
        setModalOpen(false);
        setEditingEntry(null);
    };
    ...
    if (!ready) {
        return <div className="p-8">Loading TOK data...</div>;
    }
        ...
        <div className="flex flex-col gap-6 pb-32 pt-8 px-4 md:px-8 max-w-5xl mx-auto">
            ...
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                <div className="flex flex-col justify-between overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-900 p-5 shadow-lg dark:border-zinc-800 dark:bg-zinc-50">
                    <p className="font-sans text-[10px] font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Total Entries</p>
                ...
                {tokEntries.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-200 bg-zinc-50 py-24 text-center dark:border-zinc-800 dark:bg-zinc-900/50">
                        <p className="font-sans text-lg font-medium text-zinc-500">
                            No TOK entries yet. Add your reflections to build your journal.
                        </p>
                    </div>
                ) : (
                    tokEntries.map((entry) => (
                            ...
                            {entry.evidenceUri && (
                                <div className="mt-1">
                                    <a
                                        ...
                                        rel="noreferrer"
                                    >
                                        <ExternalLink className="mr-1 h-3 w-3" /> View Evidence Link
                                    </a>
                                </div>
                            )}
                        </div>
                ...
                <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle className="font-display text-2xl font-bold">
                            {editingEntry ? "Edit TOK Entry" : "New TOK Entry"}
                        </DialogTitle>
                    </DialogHeader>
                    <TokForm
                        entry={editingEntry}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 pt-4">
            ...
            <div className="flex flex-col gap-2">
                ...
                <Input
                    ...
                    placeholder="e.g. Activity Reflection, Object Entry..."
                />
            </div>

            <div className="flex flex-col gap-2">
                <Label htmlFor="reflectionText" className="text-xs font-bold uppercase tracking-wider text-zinc-500">
                    Reflection / Journal <span className="text-zinc-400 normal-case font-medium">(Required)</span>
                </Label>
                <Textarea
                    id="reflectionText"
                    ...
                    placeholder="What did you learn? How do you relate this to the core themes?"
                />
            </div>

            <div className="flex flex-col gap-2">
                <Label htmlFor="evidenceUri" className="text-xs font-bold uppercase tracking-wider text-zinc-500">
                    Link to Material <span className="text-zinc-400 normal-case font-medium">(Optional)</span>
                </Label>
                <Input
                    id="evidenceUri"
                    type="url"
